...
/**
 * @title GastronomiaElSalvador
 * @dev Registro historico con Likes, Dislikes e Identificador de Masa (Maiz/Arroz).
 */
contract GastronomiaElSalvador {

    struct Plato {
        string nombre;
        string descripcion;
        string tipoMasa; // Ej: Maiz, Arroz, Yuca
        uint256 likes;
        uint256 dislikes;
    }
    ...
    constructor() {
        // Inauguramos con las Pupusas Revueltas
        registrarPlato(
            "Pupusas Revueltas", 
            "Tortilla gruesa rellena de chicharron molido, frijoles refritos y queso fundido.",
            "Maiz"
        );
    }
    ...
    function consultarPlato(uint256 _id) public view returns (
        ...
        string memory tipoMasa,
        uint256 likes, 
        uint256 dislikes
    ) {
        require(_id > 0 && _id <= totalPlatos, "ID invalido.");
        Plato storage p = menuHistorico[_id];
        return (p.nombre, p.descripcion, p.tipoMasa, p.likes, p.dislikes);
    }
}
